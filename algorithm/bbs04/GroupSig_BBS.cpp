...
namespace eth
{
    ...
    int BBSGroupSig::create_group(string& result,const std::string& param)
    {
        ...
        update_linear(pbc_param_setting);  // pbc_param_setting = {"linear_type":"f", "bit_len":256}
...
//};
        ...
        if( init_succ != SUCCESS)
            return init_succ;
        LOG(DEBUG)<<"CREATE_GROUP_KEY, signature_len:"<<sys_param->signature_length;   // len = 384
